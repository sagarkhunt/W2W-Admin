<template>
	<div
		...
		:class="[{ 'h-nav-group-open': openItems }, { 'h-nav-group-active': open }, { 'disabled-item pointer-events-none': group.isDisabled }]"
		@mouseover="mouseover"
		@mouseleave="mouseout"
	>
		<!-- Group Label -->
		<div class="group-header w-full flex items-center">
			<span class="flex items-center w-full">
				<!-- Group Icon -->
				<feather-icon v-if="group.icon || this.groupIndex > Math.floor(this.groupIndex)" :icon="group.icon || 'CircleIcon'" :svgClasses="iconClasses" />
			...
			<!-- Group Collapse Icon -->
			<feather-icon :class="[{ rotate90: openItems }, 'feather-grp-header-arrow']" :icon="bottom ? 'ChevronDownIcon' : $vs.rtl ? 'ChevronLeftIcon' : 'ChevronRightIcon'" svg-classes="w-4 h-4" />
		</div>
		<!-- /Group Label -->
		...
		<transition name="fade-bottom-2x">
			<ul :style="styleItems" class="h-nav-group-items h-nav-menu-dd absolute shadow-drop py-2" v-show="openItems" ref="childDropdown">
				<li v-for="(groupItem, index) in group.submenu" :key="index">
					<h-nav-menu-group v-if="groupItem.submenu" :group="groupItem" :groupIndex="Number(`${groupIndex}.${index + 1}`)" :open="isGroupActive(groupItem)" :openHover="openHover" />
