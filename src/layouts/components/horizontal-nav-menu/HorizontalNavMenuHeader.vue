<template>
	<div class="nav-header py-4" @mouseover="hovered" @mouseleave="hovered(false)" @click="showChildren = !showChildren">
		<!-- header -->
		<div
			class="header-label flex items-center cursor-pointer relative px-5 py-2"
			:class="[{ 'text-white bg-primary-gradient header-active': isHeaderActive }, { 'header-open': isHovered || showChildren }]"
		>
		...
		<transition name="fade-top-2x">
			<ul class="header-children h-nav-menu-dd shadow-drop text-intial absolute shadow-lg py-2" :class="{ 'dd-right': dropRight }" v-show="showChildren" ref="headerDropdown">
				<li v-for="(item, index) in header.items" :key="item.name">
					...
					<template v-else>
						<h-nav-menu-group openHover :key="`group-${index}`" :group="item" :groupIndex="index" :open="checkGrpChildrenActive(item)" />
					</template>
				</li>
			</ul>
		</transition>
