<template>
  <div class="send__email">
    <vs-popup button-accept="false" button-cancel="false" :active.sync="isActive" :title="$t('model.SendMail')">
      <div class="con-exemple-prompt">
        ...
        <!-- email -->
        <div>
          <vs-input placeholder="Name" vs-placeholder="Email" v-model="userMailInfo.email" class="mt-3 w-full" />
        </div>
...
export default {
  ...
  mounted() {
    this.userMailInfo.full_name = this.jobseeker.full_name
    this.userMailInfo.email = this.jobseeker.email
  },
  computed: {
    // modal stat
    isActive: {
  ...
  methods: {
    async sendMail() {
      ...
      this.$store
        .dispatch('jobseeker/sendMailForJobseeker', { ...this.userMailInfo })
        .then(({ message }) => {
          this.isActive = false
          this.$vs.notify({
            title: 'Success',
