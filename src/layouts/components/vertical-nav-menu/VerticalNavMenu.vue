  ...
  <div class="parentx">
    ...
    >
      <div @mouseenter="mouseEnter" @mouseleave="mouseLeave">
        <!-- Header -->
        <div class="header-sidebar flex justify-between items-center" slot="header">
          <!-- Logo -->
          <router-link tag="div" class="vx-logo cursor-pointer flex items-center" to="/">
            <img src="@/assets/images/logo/logo.png" class="w-10 mr-4 fill-current text-primary" />
          ...
          <div>
            ...
            <template v-else-if="!showCloseButton && !verticalNavMenuItemsMin">
              <feather-icon id="btnVNavMenuMinToggler" class="mr-0 cursor-pointer" :icon="'MenuIcon'" svg-classes="stroke-current text-primary" @click="toggleReduce(!reduce)" />
            </template>
          </div>
          <!-- /Menu Toggle Buttons -->
        </div>
...
export default {
  ...
  computed: {
    ...mapGetters('auth', ['checkPermissionByIds', 'checkRolesByIds']),
    isGroupActive() {
      return (item) => {
    ...
    menuItemsUpdated() {
      ...
      for (const [index, item] of this.navMenuItems.entries()) {
        ...
        if (!(this.checkRolesByIds(item.roles || []) || this.checkPermissionByIds(item.permissions || []))) {
        ...
        if (item.header && item.items.length && (index || 1)) {
          ...
          for (const [_, subItem] of item.items.entries()) {
            if (!(this.checkRolesByIds(subItem.roles || []) || this.checkPermissionByIds(subItem.permissions || []))) {
              continue
            }
  ...
  watch: {
    $route() {
      if (window.innerWidth > 1199) {
        ...
        if (this.isVerticalNavMenuActive && this.showCloseButton) this.$store.commit('TOGGLE_IS_VERTICAL_NAV_MENU_ACTIVE', true)
      }
    },
    reduce(val) {
