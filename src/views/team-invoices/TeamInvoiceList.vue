<template>
  <div class="vs-con-loading__container table-padding team_invoice">
    <div class="mb-6 desc">{{ $t('here_is_a_list_of_team_invoices_you_can_manage_like_approval,_vault,_etc') }}</div>
    <div class="vx-card md:p-8 p-6 md:pb-1">
      ...
      >
        <template slot="thead">
          ...
          <vs-th sort-key="team_name "> {{ $t('team_group') }}</vs-th>
          <vs-th>{{ $t('vault') }}</vs-th>
        ...
        <template slot="header">
          ...
          <div class="flex lg:flex-no-wrap flex-wrap items-end mb-3 w-full invoice-dropdown-wrap gap-2">
            <div class="flex btn-gap">
              <div
                @click="$router.push('/team-invoices/vault-invoices/list')"
        ...
        <template slot-scope="{ data }" ref="tableBody">
          <vs-tr :data="tr" :key="i" v-for="(tr, i) in data">
            <vs-checkbox class="mt-8 ml-3" v-model="selectedIds" :vs-value="tr.id"></vs-checkbox>
            ...
            <vs-td>
              <vs-td>
                <p @click="viewSingleEntrepreneur(tr.team && tr.team.created_by && tr.team.created_by.user && tr.team.created_by.user.id, tr.team && tr.team.created_by && tr.team.created_by.id)">
                  {{ (tr.team.created_by.user && tr.team.created_by.user.full_name) || '-' }}
                </p>
              </vs-td>
            </vs-td>
            <vs-td> € {{ tr.company_pay_amount.toFixed(2) }} </vs-td>
            <vs-td>
              <div v-if="tr.team && tr.team.team_name">
                {{ tr.team.team_name || '-' }}
              </div>
              <div v-else>-</div>
            </vs-td>
            ...
            <vs-td>
              <vs-switch color="primary" v-model="tr.isArchived" vs-value="active" @input="updateArchivedTeamInvoice(tr.id, tr.isArchived)">
                <span slot="on">{{ $t('true') }}</span>
                <span slot="off">{{ $t('false') }}</span>
              </vs-switch>
            </vs-td>
            ...
            <vs-td>
              <vs-switch color="primary" v-model="tr.markAsWrong" vs-value="active" @input="updateMarkAsWrongInvoice(tr.id, tr.markAsWrong)">
                <span slot="on">{{ $t('true') }}</span>
                <span slot="off">{{ $t('false') }}</span>
              </vs-switch>
            </vs-td>
            <vs-td>
              <div class="flex items-center justify-center" :style="{ direction: $vs.rtl ? 'rtl' : 'ltr' }">
                <vx-tooltip :text="$t('tooltip.ViewRecord')">
                  <feather-icon icon="EyeIcon" svgClasses="h-5 w-5 mr-4 hover:text-primary cursor-pointer" @click="viewTeamInvoiceRecord(tr)" />
                </vx-tooltip>
              </div>
            </vs-td>
            <vs-td>
    ...
    <!-- edit team invoice -->
    <editTeamInvoice v-if="isEditTeamInvoiceModalMounted" :isModalShow.sync="isEditTeamInvoiceModalShow" @newInvList="getData()" :invoiceId="invoiceId" />
    <!-- end edit team invoice -->
    ...
    <vs-popup button-accept="false" button-cancel="false" :active.sync="downloadInvoicePopup" :title="$t('model.TotalDownloadSize')">
      <div class="vs-con-loading__container">
        <div class="vx-card p-6 pb-10 pt-10 pl-10">
          <vs-table class="vs-con-loading__container" stripe :sst="true" maxHeight="500px" :data="teamInvoiceData">
            <template slot="thead">
              <vs-th>{{$t('sr')}} </vs-th>
              ...
              <vs-tr :data="record" :key="i" v-for="(record, i) in invoiceFiles.invoice_files">
                ...
                <vs-td>
                  <vx-tooltip :text="$t('tooltip.DownloadContract')">
                    <a href="path_to_file" :download="record.file_path"><feather-icon icon="DownloadIcon" svgClasses="h-5 w-5 mr-4 hover:text-primary cursor-pointer" /> </a
                  ></vx-tooltip>
                </vs-td>
              </vs-tr>
            </template>
...
import AddRefNumber from '@/views/team-invoices/components/AddRefNumber'
import EditRefNumber from '@/views/team-invoices/components/EditRefNumber'
import sendRevisonRecordTeamInvoice from '@/views/team-invoices/components/SendForRevisionTeamInvoice'
import editTeamInvoice from '@/views/team-invoices/components/EditTeamInvoice'
...
export default {
  ...
  data() {
    return {
      ...
      //edit team invoice
      isEditTeamInvoiceModalMounted: false,
      isEditTeamInvoiceModalShow: false,
  ...
  computed: {
    ...mapState('teaminvoice', ['loading', 'totalTeamInvoice', 'TeamInvoice', 'teamInvoiceFilteredCount', 'teamInvoicedRecords', 'SearchByTypes', 'QuickPayTypes']),
    teamInvoiceData() {
  ...
  methods: {
    ...mapActions('teaminvoice', {
      ...
      updateTeamInvoiceMarkAsWrong: 'updateTeamInvoiceMarkAsWrong',
      addFileInvoice: 'addFileInvoice',
      deleteTeamInvoiceRecord: 'deleteTeamInvoiceRecord',
      archivedTeamInvoicesBulk: 'archivedTeamInvoicesBulk',
      verifyBlukTeamInvoice: 'verifyBlukTeamInvoice',
    ...
    async archivedInvoice() {
      await this.archivedTeamInvoicesBulk({
    ...
    // For Archived Invoices
    async verifyInvoice() {
      await this.verifyBlukTeamInvoice({
        invoiceId: this.selectedIds
      })
      this.getData()
      this.selectedIds = []
    ...
    createTeamInvoice() {
      this.$router.push(`/team-invoices/create`)
    },
    EditTeamInvoice(id) {
      this.$router.push(`/team-invoices/${id}/edit`)
    },
    openInvoice() {
    ...
    //add ref number
    addHeerosID(id) {
      this.isAddRefNumberModalMounted = true
      this.isAddRefNumberModalShow = true
      this.refNumberId = id
    },
    //update ref number
    ...
    openInvoiceData(tr, id) {
      ...
      this.isEditTeamInvoiceModalMounted = true
      this.isEditTeamInvoiceModalShow = true
    },
    ...
    deleteRecord(id) {
      this.$vs.dialog({
        ...
        accept: () => this.deleteTeamInvoice(id),
        acceptText: 'Delete'
      })
    },
    async deleteTeamInvoice(id) {
      try {
        const { message } = await this.deleteTeamInvoiceRecord(id)
        this.$vs.notify({
    ...
    //update mark as wrong
    async updateMarkAsWrongInvoice(id, mark_as_wrong) {
      try {
        const { message } = await this.updateTeamInvoiceMarkAsWrong({
          id,
          mark_as_wrong
        })
        this.$vs.notify({
    ...
    async updateArchivedTeamInvoice(id, status) {
      try {
  ...
  watch: {
    ...
    isEditTeamInvoiceModalShow: {
      immediate: true,
      handler(newVal) {
        if (!newVal) {
          setTimeout(() => {
            this.isEditTeamInvoiceModalMounted = false
          }, 0)
        }
      }
    },
  ...
  beforeRouteLeave(to, from, next) {
    this.isAddRefNumberModalShow = false
    this.isAddFileModalShow = false
    this.isEditRefNumberModalShow = false
    setTimeout(() => {
      next()
    }, 0)
  }
