<template>
  <div id="theme-customizer">
    ...
    <vs-button @click.stop="active = !active" color="primary" type="filled" class="customizer-btn" icon-pack="feather" icon="icon-settings" />
    ...
    <vs-sidebar click-not-close hidden-background position-right v-model="active" class="items-no-padding">
      <div class="h-full">
        <div class="customizer-header mt-6 flex items-center justify-between px-6">
          <div>
            <h4>{{ $t('theme_customizer') }}</h4>
            <small>{{ $t('customize_&_preview_in_real_time') }}</small>
          </div>
        ...
        <component :is="scrollbarTag" class="scroll-area--customizer pt-4 pb-6" :settings="settings" :key="$vs.rtl">
          <div class="px-6">
            <!-- Layout Type -->
            <div class="mt-4">
              <h5 class="mb-2">{{ $t('layout_type') }}</h5>
              <div>
                <vs-radio v-model="layoutType" vs-value="vertical" class="mr-4" vs-name="layout-type-vertical">{{ $t('vertical') }}</vs-radio>
                <vs-radio v-model="layoutType" vs-value="horizontal" class="mr-4" vs-name="layout-type-horizontal">{{ $t('horizontal') }}</vs-radio>
              </div>
            </div>
            ...
            <!-- THEME COLORS -->
            <div>
              <h5 class="mb-4">{{ $t('theme_color') }}</h5>
              <ul class="clearfix">
                <!-- Color Square -->
                <li
            ...
            <!-- THEME -->
            <div class="mt-4">
              <h5 class="mb-2">{{ $t('theme_mode') }}</h5>
              <div>
                <vs-radio v-model="themeMode" vs-value="light" class="mr-4" vs-name="theme-mode-light">{{ $t('light') }}</vs-radio>
                <vs-radio v-model="themeMode" vs-value="dark" class="mr-4" vs-name="theme-mode-dark">{{ $t('dark') }}</vs-radio>
                <vs-radio v-if="layoutType === 'vertical'" v-model="themeMode" vs-value="semi-dark" vs-name="theme-mode-semi-dark">{{ $t('semi_dark') }}</vs-radio>
              </div>
            </div>
            ...
            <!-- NAVBAR TYPE -->
            <div class="mt-4">
              <h5 class="mb-2">{{ (layoutType === 'vertical' || windowWidth &lt; 1200) ? "Navbar" : "Nav Menu" }} {{ $t('type') }}</h5>
              <div>
                <vs-radio v-if="layoutType === 'vertical' || windowWidth < 1200" v-model="navbarTypeLocal" vs-value="hidden" class="mr-4" vs-name="navbar-type-hidden">{{ $t('hidden') }}</vs-radio>

                <vs-radio v-model="navbarTypeLocal" vs-value="static" class="mr-4" vs-name="navbar-type-static">{{ $t('static') }}</vs-radio>
                <vs-radio v-model="navbarTypeLocal" vs-value="sticky" vs-name="navbar-type-sticky" class="mr-4">{{ $t('sticky') }}</vs-radio>
                <vs-radio v-model="navbarTypeLocal" vs-value="floating" vs-name="navbar-type-floating">{{ $t('floating') }}</vs-radio>
              </div>
            </div>
            ...
            <!-- FOOTER TYPE -->
            <div class="mt-4">
              <h5 class="mb-2">{{ $t('footer_type') }}</h5>
              <div>
                <vs-radio v-model="footerTypeLocal" vs-value="hidden" class="mr-4" vs-name="footer-type-hidden">{{ $t('hidden') }}</vs-radio>
                <vs-radio v-model="footerTypeLocal" vs-value="static" class="mr-4" vs-name="footer-type-static">{{ $t('static') }}</vs-radio>
                <vs-radio v-model="footerTypeLocal" vs-value="sticky" vs-name="footer-type-sticky">{{ $t('sticky') }}</vs-radio>
              </div>
            </div>
            ...
            <!-- SHOW SCROLL TO TOP -->
            <div class="mt-4 flex justify-between">
              <h5 class="mb-2">{{ $t('hide_scroll_to_top') }}</h5>
            ...
            <!-- ROUTER ANIMATION -->
            <div class="mt-4">
              <h5 class="mb-2">{{ $t('router_animation') }} {{ routerTransitionLocal }}</h5>
