<template>
  <div class="negative_teamInvoice">
    <div class="mb-6 desc">{{ $t('here_you_can_see_all_marked_as_wrong_or_negative_team_invoices') }}</div>
    <div class="vx-card md:p-8 p-6 md:pb-1">
      ...
      >
        <template slot="thead">
          <vs-th>{{ $t('sr') }}</vs-th>
          <vs-th sort-key="invoice_to">{{ $t('company_nameF') }} </vs-th>
          ...
          <vs-th sort-key="verify">{{ $t('verify') }} </vs-th>
        ...
        <template slot-scope="{ data }" ref="tableBody">
          <vs-tr :data="tr" :key="i" v-for="(tr, i) in data">
            ...
            <vs-td>€ {{ tr.company_pay_amount || '-' }}</vs-td>
            ...
            <vs-td>
              <vs-switch color="primary" v-model="tr.markAsWrong" vs-value="active" @input="updateStatusMarkAsWrongNegativeTeam(tr.id, tr.markAsWrong)">
                <span slot="on">{{ $t('true') }}</span>
                <span slot="off">{{ $t('false') }}</span>
              </vs-switch>
            </vs-td>
...
export default {
  ...
  methods: {
    ...
    async updateStatusMarkAsWrongNegativeTeam(id, mark_as_wrong) {
      try {
