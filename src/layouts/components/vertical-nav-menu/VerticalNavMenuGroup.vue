	...
	>
		...
		<div @click="clickGroup" class="group-header w-full">
			<span class="flex items-center w-full">
				<!-- Group Icon -->
				<feather-icon v-if="group.icon || this.groupIndex > Math.floor(this.groupIndex)" :icon="group.icon || 'CircleIcon'" :svgClasses="{ 'w-3 h-3': this.groupIndex % 1 != 0 }" />
		...
		<ul ref="items" :style="styleItems" class="vs-sidebar-group-items">
			<li v-for="(groupItem, index) in group.submenu" :key="index">
				<!-- If item is group -->
				<v-nav-menu-group v-if="groupItem.submenu" :group="groupItem" :groupIndex="Number(`${groupIndex}.${index + 1}`)" :open="isGroupActive(groupItem)" :openHover="openHover" />

				<!-- Else: Item -->
				<v-nav-menu-item
					icon-small
					v-else-if="checkAccess(groupItem)"
...
export default {
	...
	methods: {
		checkAccess(groupItem) {
			if (!(this.checkRolesByIds(groupItem.roles || []) || this.checkPermissionByIds(groupItem.permissions || []))) {
				return false
			}
			return true
		},
			...
			if (!this.openHover) {
				...
				this.$parent.$children.map((child) => {
					if (child.isGroupActive) {
						...
						if (child !== this && child.open && this.maxHeight !== '0px') {
							setTimeout(() => {
								child.maxHeight = `${0}px`
							}, 50)
						}
