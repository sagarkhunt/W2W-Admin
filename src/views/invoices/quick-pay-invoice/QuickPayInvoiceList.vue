<template>
  <div id="table-loading" class="vs-con-loading__container quick_invoice">
    <div class="mb-6 desc">{{ $t('here_you_can_see_invoice_that_are_applied_for_quickpay') }}</div>
...
export default {
  ...
  computed: {
    ...mapState('invoice', ['loading', 'totalQuickPay', 'FilteredCountQuickPay', 'QuickPayInvoiceList', 'SearchByTypes', 'QuickPayTypes'])
  },
  methods: {
    ...mapActions('invoice', {
      getAllQuickPayInvoiceRecord: 'getAllQuickPayInvoiceRecord'
    }),
    ...
    getData() {
      this.getAllQuickPayInvoiceRecord({
        limit: this.length,
        page: this.page,
        search: this.search,
