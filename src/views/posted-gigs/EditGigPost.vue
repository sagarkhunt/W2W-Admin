      ...
      <div class="vx-row pt-5 px-5">
        ...
        <div class="vx-col w-full md:w-1/2 px-8">
          ...
          <div class="vx-row mb-2">
            ...
            <vs-input id="permanent_address" class="w-full" name="gig title" v-model="form.job_title" v-validate="'required|max:60'" />
          ...
          <div class="vx-row mb-2">
            ...
            <vs-input id="permanent_address" class="w-full" name="experience required" v-model="form.experience" v-validate="'required'" />
            <span class="text-primary text-sm" v-show="errors.has('experience required')">{{ errors.first('experience required') }}</span>
          </div>
        ...
        <div class="form-row mt-5 skill-box">
          <!-- skills -->
          ...
          <div class="form-group col-xl-4 col-lg-6 mt-5">
            ...
            <v-select v-model="tempSkill" :taggable="true" :pushTags="true" :reduce="(option) => option.id" :options="skillOption" v-on:input="addSkill(tempSkill)" />
          </div>
        </div>
      </div>
      ...
      <div class="vx-row">
        <div class="vx-col w-full">
          <div class="mt-4 lg:float-right text-center">
            <vs-button class="mr-2 vs-con-loading__container" id="edit-gig-post" type="filled" @click="save_changes" :disabled="!validateForm">{{ $t('update_post') }}</vs-button>
...
export default {
  ...
  async mounted() {
    await this.$store
      ...
      .then((res) => {
        this.countryOptions = res.data.map((x) => ({ label: x.country_name, value: x.referenceId }))
      })
      .catch((err) => {
        const { message } = err
        this.$vs.notify({
    ...
    await this.$store
      .dispatch('postedJobs/getGigPostById', this.$route.params.id)
      .then(async (res) => {
        const data = res.data.data
        if (data.countryReferenceId) {
          await this.fetchCityByCounrtyId(data.countryReferenceId)
        }

        this.skillNameOptions = data.skills.reduce((acc, item) => {
          acc.push(item.reference && item.reference.skill)
          return acc
        }, [])

        this.form.industryId = data.industryReferenceId
        this.form.companyId = data.companyId
        this.form.vacancies = data.vacancies
        this.form.job_mode = data.job_mode
        this.form.salary_offer = data.salary_offer
        this.form.countryId = data.countryReferenceId
        this.form.job_status = data.job_status
        this.form.job_description = data.job_description
        this.form.job_title = data.job_title
        this.form.experience = data.experience
        this.form.job_duration = data.job_duration
        this.form.apply_before = data.apply_before
        this.form.address = data.address
        this.form.cityId = data.cityReferenceId
        this.form.zip_code = data.zip_code

        this.form.job_permits = data.permits.reduce((acc, item) => {
          acc.push(item.reference && item.reference.referenceId)
          return acc
        }, [])

        this.form.job_skills = data.skills.reduce((acc, item) => {
          acc.push(item.reference && item.reference.referenceId)
          return acc
        }, [])
      })
  ...
  methods: {
    ...mapActions('postedJobs', { udateGigPost: 'udateGigPost' }),
    ...
    async save_changes() {
      ...
      this.form.apply_before = dateMixins.methods.formatOldDate(this.form.apply_before)
      try {
        const { message } = await this.udateGigPost({
    ...
    fetchCityByCounrtyId(id) {
      this.$store.dispatch('getCommonCities', id).then((res) => {
        this.cityOptions = res.data.reduce((acc, item) => {
          acc.push({ label: item.city_name, value: item.referenceId })
          return acc
        }, [])
      })
    },
  ...
  watch: {
    loading() {
      if (this.loading) {
        this.$vs.loading({
          container: '#edit-gig-post',
          scale: 0.45
        })
      } else {
        this.$vs.loading.close('#edit-gig-post > .con-vs-loading')
      }
    },
    ...
    'form.countryId': {
      ...
      handler() {
        if (this.form.countryId) {
          this.cityOptions = []
          this.cityId = null
          this.fetchCityByCounrtyId(this.form.countryId)
        }
      }
    }
  }
