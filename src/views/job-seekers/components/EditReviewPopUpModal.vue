<template>
  <div>
    <!-- Deductions Info -->
    <vs-popup button-accept="false" button-cancel="false" :active.sync="isActive" :title="$t('model.EditReview')">
      <div class="con-exemple-prompt">
        ...
        <div class="">
          ...
          <div class="mb-2">
            <select-2
              class="w-full"
              :value="edit_review_form.companyId"
              @input="(item) => (edit_review_form.companyId = item && item.value)"
              name="Reviwer"
        ...
        <div class="mt-3">
          ...
          <div class="mb-2">
            <vs-input id="job_title" class="w-full" name="job_title" v-model="edit_review_form.job_title" disabled />
          </div>
        </div>
        ...
        <div class="mt-2">
          ...
          <vs-textarea id="comment" class="w-full" name="Review text" v-validate="'max:255'" v-model="edit_review_form.comment" />
      ...
      <div class="footer text-right">
        ...
        <button class="mt-2 ml-2 text-sm vs-component vs-button vs-button-success vs-button-filled p-3" @click="edit_seeker_review" type="relief" :disabled="!validateForm">
          {{ $t('update') }}
        </button>
      </div>
    </vs-popup>
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch('jobseeker/getSingleReviewRecord', this.reviewId).then((res) => {
      const data = res.data.data
      this.edit_review_form.companyId = data.companyId
      this.edit_review_form.job_title = data.job_title
      this.edit_review_form.comment = data.comment
      this.star = data.rate
    })
  },
  computed: {
  ...
  methods: {
    ...mapActions('jobseeker', {
      updateRecordReviewGigs: 'updateRecordReviewGigs'
    }),
    ...
    async getReviewGigList() {
      const { data } = await this.$store.dispatch('jobseeker/getReviewerGigName', { seekerId: this.$route.params.id, companyId: this.edit_review_form.companyId })
      this.reviwerGigOption = data
    },

    async edit_seeker_review() {
      if (!(await this.$validator.validate())) {
        return false
      }
      this.edit_review_form.rate = this.star
      try {
        const { message } = await this.updateRecordReviewGigs({
          id: this.reviewId,
          data: {
            ...this.edit_review_form
          }
        })
        this.$emit('done')
        this.isActive = false
  ...
  watch: {
    ...
    'edit_review_form.companyId': {
      immediate: true,
      handler() {
        if (this.edit_review_form.companyId) {
