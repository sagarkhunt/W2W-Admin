<template>
	<div class="vx-breadcrumb relative" >
		<ul class="flex flex-wrap items-center">
			<li class="inline-flex items-end">
				<router-link to="/">
					<feather-icon icon="HomeIcon" svgClasses="h-5 w-5 mb-1 stroke-current text-primary" />
				</router-link>
				<span class="breadcrumb-separator mx-2"><feather-icon :icon="isRTL ? 'ChevronsLeftIcon' : 'ChevronsRightIcon'" svgClasses="w-4 h-4" /></span>
			</li>
			<li v-for="(link, index) in route.meta.breadcrumb.slice(1, -1)" :key="index" class="inline-flex items-center">
				...
				<span class="breadcrumb-separator mx-2 flex items-start"><feather-icon :icon="isRTL ? 'ChevronsLeftIcon' : 'ChevronsRightIcon'" svgClasses="w-4 h-4" /></span>
			</li>
