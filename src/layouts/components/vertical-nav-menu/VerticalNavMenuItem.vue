<template>
	<div class="vs-sidebar--item" :class="[{ 'vs-sidebar-item-active': activeLink }, { 'disabled-item pointer-events-none': isDisabled }]">
		<router-link tabindex="-1" v-if="to" exact :class="[{ 'router-link-active': activeLink }]" :to="to" :target="target">
			<vs-icon v-if="!featherIcon" :icon-pack="iconPack" :icon="icon" color="text-primary" />
			<feather-icon v-else :class="{ 'w-5 h-5': iconSmall, 'text-primary': !activeLink }" :icon="icon" />
			<slot />
		</router-link>

		<a v-else :target="target" :href="href" tabindex="-1">
			<vs-icon v-if="!featherIcon" :icon-pack="iconPack" :icon="icon" color="text-primary" />
			<feather-icon v-else :class="{ 'w-5 h-5': iconSmall }" :icon="icon" class="text-primary" />
			<slot />
		</a>
	</div>
</template>
